...
com = random.randint(1, 30)  # 1 ~ 30 중 하나의 정수를 추출
print('<< 1 ~ 30 숫자 맞추기 게임 >>')
while True:
    player = int(input('숫자입력(종료 0): '))
    if player == 0:
        break
    elif player == com:
        print('정답!!')
...
# ramdom.sample(범위, 개수) --> 범위에서 갯수만큼 중복되지 않는 수를 추출
...
words = ["사과", "바다", "하늘", "나무", "학교", "고양이", "자동차", "음악", "친구", "행복"]
input('타자게임 시작(엔터 입력)')
